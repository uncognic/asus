...
static void cmd_get(const char *target) {
    ...
    } else if (strcmp(target, "battery-level") == 0 || strcmp(target, "bl") == 0) {
    ...
    } else if (strcmp(target, "battery-status") == 0 || strcmp(target, "bs") == 0) {
        if (sysfs_read(BATTERY_STATUS_PATH, buf, sizeof(buf)) == 0) {
            printf("%s\n", buf);
        }
    } else {
